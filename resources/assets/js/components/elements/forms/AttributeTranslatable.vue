<script>
    export default {
        ...
        props: {
            params: {
                type: Object,
                default: {'translating':false, 'channel': 'ecommerce', 'language':'en'}
            },
            fields: {
                type: Object,
                default: {}
        ...
        methods: {
            ...
            isDefault() {
                return (this.params.channel === this.defaults.channel && this.params.language === this.defaults.language) ? true : false;
            },
            useDefault: function(obj) {
                if(obj.checked) {
                    this.fields[obj.id].value[this.params.channel][this.params.language] = null;
                } else {
                    this.fields[obj.id].value[this.params.channel][this.params.language] = this.originalFields[obj.id].value[this.params.channel][this.params.language];
                }
            }
        },
        created: function() {
            ...
            <div class="col-xs-12 form-group" :class="{ 'col-md-6': params.translating }">
                <div class="form-group" v-for="(field, key) in fields">
                    ...
                    <div v-if="field.type == 'text'">
                        <candy-input
                                :id="'default-'+ key"
                                v-model="field.value[defaults.channel][defaults.language]">
                        </candy-input>
                    </div>
